...
describe("save a character as an owned copy", () => {
    ...
    it("creates a fresh identity, activates it, caches ownership and correlates source and copy", async () => {
        const saved = {
            ...
            data: { ...source.character, id: "", characterVersion: 0 },
            version: 10,
            characterVersion: 0,
            createdAt: "today",
            updatedAt: "today"
        ...
        expect(client.getQueryData(["characters", "copy"])).toMatchObject({
            canEdit: true,
            shared: false
        })
        expect(client.getQueryData(["characters"])).toEqual([
            { id: "original", shared: true },
            expect.objectContaining({ id: "copy", shared: false })
        ])
        expect(posthog.capture).toHaveBeenCalledWith(
            "character_save_as_copy_requested",
            expect.objectContaining({
                source_character_id: "original",
                source_owner_id: "owner",
                viewer_user_id: "viewer"
            })
        )
        expect(posthog.capture).toHaveBeenCalledWith(
            "character_save_as_copy_applied",
            expect.objectContaining({ new_character_id: "copy", source_character_id: "original" })
        )

...
const ShareCharacterModalContent = ({
    ...
    const { data: shares, isLoading: isLoadingShares } = useCharacterShares(
    ...
    return (
        <Stack gap="md">
            ...
            <Text c="dimmed" size="sm">
                This is a read-only share. The other user can view your character, and they can
                store and edit their own copy of it.
            </Text>
            <Text c="dimmed" size="sm">
...
// * Remove mutation onSuccess callbacks (deprecated) and use state managed inside of useMutation instead of doing it on your own
// * Clean up all the useStates we have in here, most of them can probably be managed with just query state
...
const MePage = () => {
    ...
    const performLoadCharacter = async (char: {
        ...
        try {
            ...
            if (beCharacter.characterVersion > feVersion) {
                // Backend version is newer - warn user they're about to load an older version
                setLoadingCharacterId(null)
                setVersionConflictInfo({
                    beVersion: beCharacter.characterVersion,
                    feVersion,
                    message: `(Loading ${char.name}): ${char.name} version in database (${beCharacter.characterVersion}) is higher than in browser (${feVersion}) - loading will overwrite your current changes with an older version.`
                })
                setCharacterToLoad({ id: char.id, name: char.name, data: char.data })
                setVersionConflictModalOpened(true)
                return
            }
        } catch (error) {
    ...
    const handleConfirmLoadJson = async () => {
        ...
        // Save current character first if it exists in the database
        if (character.id && character.name.trim()) {
            const currentCharacterExists = userCharacters.find((c) => c.id === character.id)
            if (currentCharacterExists && !currentCharacterExists.shared) {
                try {
                    ...
                    if (beCharacter.characterVersion > feVersion) {
                        // Version conflict - warn but continue with JSON load
                        notifications.show({
                            title: "Version Conflict",
                            message: `Current character "${character.name}" has a newer version in database. Saving skipped. You can manually save after loading if needed.`,
                            color: "yellow",
                            autoClose: 5000
                        })
                        // Continue with JSON load without saving
                    } else {
                        // Save current character
                        await new Promise<void>((resolve, reject) => {
                            updateCharacterMutation.mutate(
                    ...
                    console.warn("Failed to save current character before loading JSON:", error)
                    notifications.show({
                        title: "Warning",
                        message: "Failed to save current character. Loading JSON character anyway.",
                        color: "yellow"
                    })
                }
            }
            ...
            },
            {
                ...
                onError: (error: unknown) => {
                    notifications.show({
                        title: "Error",
                        message:
                            error instanceof Error ? error.message : "Failed to unshare character",
                        color: "red"
                    })
                }
            }
            ...
            { name: newCoterieName.trim() },
            {
                ...
                onError: (error) => {
                    notifications.show({
                        title: "Error",
                        message:
                            error instanceof Error ? error.message : "Failed to create coterie",
                        color: "red"
                    })
                }
            }
            ...
            },
            {
                ...
                onError: (error) => {
                    notifications.show({
                        title: "Error",
                        message:
                            error instanceof Error ? error.message : "Failed to update coterie",
                        color: "red"
                    })
                }
            }
    ...
    return (
        <>
                    ...
                    <Select
                        ...
                        data={ownedCharacters
                            .filter((char) => {
                                ...
                                return !selectedCoterieForAdd.members.some(
            ...
            <ConfirmActionModal
                ...
                body={
                    <>
                        Are you sure you want to delete <strong>{coterieToDelete?.name}</strong>?
                        This action cannot be undone.
                    </>
                }
                confirmLabel="Delete"
            ...
            >
                <Stack gap="md">
                    <Text>
                        ...
                        <strong>{characterToLoad?.name || character.name}</strong>. Loading this
                        character will discard your current unsaved changes.
                    </Text>
                    <Text c="dimmed" size="sm">
...
const CoterieSummaryContent = ({ members, theme }: CoterieSummaryContentProps) => {
    const getDisciplineCategory = (
        ...
        const physicalDisciplines: DisciplineName[] = [
            "celerity",
            "potence",
            "fortitude",
            "protean"
        ]
        const socialDisciplines: DisciplineName[] = [
            "presence",
            "dominate",
            "obfuscate",
            "animalism"
        ]
        const mentalDisciplines: DisciplineName[] = [
            "auspex",
            "blood sorcery",
            "oblivion",
            "thin-blood alchemy"
    ...
    const calculateCharacterStrength = (
        ...
        const dominant =
            physicalScore > socialScore && physicalScore > mentalScore
                ? "physical"
                : socialScore > mentalScore
                  ? "social"
                  : "mental"
                        ...
                        <Grid.Col span={{ base: 12, md: 4 }}>
                            ...
                            <Stack gap="xs">
                                {physicalSkills.map((skill) => {
                                    ...
                                    const specialties = character.skillSpecialties.filter(
                                        (s) => s.skill === skill
                                    )
                                    return (
                                        <div key={skill}>

...
for (const viewport of [
    ...
    test.describe(`character recovery at ${viewport.width}px`, () => {
        ...
        test.beforeEach(async ({ page }) => {
            ...
            await cookieClose.waitFor({ state: "visible" })
            await page.getByRole("button", { name: "Decline", exact: true }).click()
        })

        test("previews and confirms selective repair, keeps backup, and survives reload", async ({
            page
        }, testInfo) => {
            ...
            await expect(
                page.getByText("Automatic repair may cause partial data loss")
            ).toBeVisible()
            await expect(page.getByText("Flaw: Invalid flaw will be removed")).toBeVisible()
            await expect(
                page.getByText('Attributes → Strength will be reset from "invalid" to 1')
            ).toBeVisible()
            await page.screenshot({
                path: testInfo.outputPath("repair-preview.png"),
                fullPage: true
            })
            expect(
                await page.evaluate(() =>
                    JSON.parse(localStorage.getItem("character_broken_save")!)
                )
            ).toBe(original)
            await page.getByRole("button", { name: "Cancel repair" }).click()
            await expect(
                page.getByRole("button", { name: "Confirm repair and load character" })
            ).toHaveCount(0)
            await page.getByRole("button", { name: "Preview automatic repair" }).click()
            ...
            const stored = await page.evaluate(() => ({
                ...
                recovery: JSON.parse(localStorage.getItem("character_recovery_saves")!)
            }))
            ...
            expect(stored.character.flaws.map((flaw: { name: string }) => flaw.name)).toEqual([
                "Valid flaw"
            ])
        ...
        test("valid characters load unchanged without showing recovery", async ({ page }) => {
            ...
            await expect(page.getByRole("dialog", { name: "Character Data Error" })).toHaveCount(0)
            expect(
                await page.evaluate(() => JSON.parse(localStorage.getItem("character")!))
            ).toEqual(expected)
            expect(
                await page.evaluate(
                    () => !!JSON.parse(localStorage.getItem("character_broken_save") || '""')

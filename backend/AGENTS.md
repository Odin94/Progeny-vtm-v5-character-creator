...
- Route or schema changes: `pnpm run build` — no TypeScript errors is the minimum bar. Then `pnpm run test:run` to confirm the health check still passes.
- Database schema changes: `pnpm run db:generate` and review the generated migration before applying.
